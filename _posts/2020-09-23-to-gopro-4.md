...
Note that when dealing with pointers, we need to keep `nil` in mind as not like primitive types or composite types, the default value of pointer is `nil`. Therefore, we need to be careful of nil pointer dereference, also known as NullPointerException in Java/C#.
...
Functions, also known as methods, are reusable logic which perform specific tasks. You can think of it as a "factory". It takes 0 to n numbers of arguments and returns 0 to n numbers of results. The structure of a function looks like this:
...
Receiver, input arguments and output arguments are three optional components we can specify when declaring a function.
...
Now we can add the `Person` type as receivers to functions. There are two form of receivers in Go, they are value receiver and pointer receiver namely.
...
Pointer receiver passes a pointer to the original value into the function. Thus, when we modify its values, it's actually going to affect the original value. Most of the time, I use pointer receivers over value receivers because it can avoid copying the receiver value. Hence, pointer receivers are more efficient. However, I want to point out that the pattern of using pointer receivers to change the value of receivers is not ideal. I would avoid using this pattern because it makes the codebase harder to maintain. We don't really want to use any dark magic in functions.

### Implementing an interface

We touched on interface in the [previous tutorial](https://medium.com/swlh/road-to-go-pro-types-structures-21e5fedc5fe0). To quickly recap, an interface is a collection of function declarations. It defines what needs to be done without telling you how to do the job.  
To implement an interface, we need to define a struct and this struct needs to have all the declared functions of that interface. When implementing required functions, we have to use either the value receiver or the pointer receiver in the function declaration.
...
Good question! We can use pointer receiver but the implication will be a bit different.  
...
Here, `*Cat` actually implemented `Animal` interface instead of `Cat` because we used pointer receivers in `Eat()` and `Drink()`.
...
Let's say that you want to store people's decisions on whether they want to go to your party. There are three options available. True means a person made the decision and the respond is positive. False means a person made the decision and the respond is negative. And nil means a person hasn't made the decision yet.  
Using `bool` as the field type is not going to meet the requirements as `bool` can only have two values, true and false. In this case, we can use pointers to introduce `nil` value into boolean type.
...
2. When you need to use receivers in functions, use pointer receivers when possible.  

As mentioned in the [function section](#valuepointer-receiver), when we have a choice between value receiver and pointer receiver. I would pick pointer receivers as they are more efficient and use less memory. However, this also brings a side effect, which is accidental mutation. Unexpected data mutations make code harder to debug and maintain. I wouldn't mutate the pointer data or make the mutation small and obvious. Like the example above, `ChangeName()` function mutates the first and last name of a person. It does a simple mutation and the function name makes the indention very obvious and clear to other people.  

3. When in need of singletons, pointers are the perfect fit.  

A [singleton](https://en.wikipedia.org/wiki/Singleton_pattern) is something we only want to initiate once in the lifetime of an application. For instance, when we need to integrate a third-party API, we might want to create a integration client for that API. This client can be a singleton. Or when we use want to cache in the application, the cache client can be a singleton, too.  
We normally initialise the singleton instance inside the `main()` function and then pass it around to different parts of the codebase. If we pass the singleton value as a function parameter, we effectively make it not a singleton anymore because passing value will result in copying the instance. Thus, we'd better pass the pointer around the application so there is truly only one instance inside the application.  
